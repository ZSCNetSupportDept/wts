...
AND ($2::text IS NULL OR u.phone ILIKE '%' || $2::text || '%')
AND ($3::wts.block[] IS NULL OR u.block = ANY($3::wts.block[]))
AND ($4::text IS NULL OR u.room ILIKE '%' || $4::text || '%')
AND ($5::wts.isp[] IS NULL OR u.isp = ANY($5::wts.isp[]))
AND ($6::text IS NULL OR u.account ILIKE '%' || $6::text || '%')
ORDER BY u.sid
`
...
func (q *Queries) GetActiveTicketCountByBlock(ctx context.Context) ([]GetActiveTicketCountByBlockRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.Block, &i.Total, &i.Today); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
